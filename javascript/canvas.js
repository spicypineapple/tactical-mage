...
function drawGrid(){
    ...
    for (var i=0; i<Grid.sx; i++) {
        for (var j=0; j<Grid.sy; j++) {
            let color;
            switch (Grid.tilemap[i+j*Grid.sx].displayValue) {
                case DisplayValue.HOVER:
                    color = '#fe5d00';
                    break;
                case DisplayValue.SELECTED:
                    color = '#ffe74b';
                    break;
                case DisplayValue.ENEMY_TARGET:
                    color = '#ff4b63';
                    break;
                case DisplayValue.ALLY_TARGET:
                    color = '#63ff4b';
                    break;
                default:
                    color = Grid.tilecolor; // #ff8d4b
                    break;
            }
            const coord = getCoordfromXY(i,j);
            drawTile(
                coord.x, // x
                coord.y, // y
                color
